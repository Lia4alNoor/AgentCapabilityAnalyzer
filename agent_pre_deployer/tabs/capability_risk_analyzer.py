...
def render_tool_metadata_and_classification(tool):
    ...
    with col_meta:
        ...
        hint_rows = [
            ("Read-only hint", tool.get("readOnlyHint")),
            ("Destructive hint", tool.get("destructiveHint")),
            ("Idempotent hint", tool.get("idempotentHint")),
            ("Open-world hint", tool.get("openWorldHint")),
            ("Source", tool.get("source")),
        ]

        for label, value in hint_rows:
            shown = "not provided" if value is None else value
...
def render():
    ...
    with st.expander("📋 Accepted file format", expanded=True):

        st.caption(
            "Upload a single JSON file containing a **list of tool objects**. "
            "The top level must be an array `[...]`, not an object. "
            "Each object in the array is checked individually and must match "
            "one of the schemas below."
        )

        format_tab_1, format_tab_2 = st.tabs(
            ["Normalized MCP format", "MCPTox format"]
        )

        with format_tab_1:
            st.markdown(
                "**Required:** `tool`, `description`  \n"
                "**Optional:** `readOnlyHint`, `destructiveHint`, "
                "`idempotentHint`, `openWorldHint`, `source` — "
                "omitted hints are treated as unset, not `false`."
            )
                ...
                """[
      {
        ...
        with format_tab_2:
            st.markdown(
                "**Required:** `tool_name`, `tool_content`  \n"
                "**Optional:** `server_name`, `query`, `security risk`, "
                "`paradigm`, `tool_address`"
            )
                ...
                """[
      {
        ...
        "query": "delete a file",
        "tool_content": "Deletes a file from the local filesystem",
        "security risk": "high",
        "paradigm": "filesystem",
        ...
        st.warning(
            "If **any single object** in the file matches neither schema, "
            "the entire analysis fails — objects aren't skipped individually. "
            "Different tool objects in the same file are allowed to use "
            "different schemas from one another, as long as each one "
            "individually satisfies one of the two above."
        )
    ...
    uploaded_file = st.file_uploader(
        ...
        help=(
            ...
            "See 'Accepted file format' above for the required schema."
        ),
    )
    ...
    if uploaded_file is None:
        ...
        st.info(
            "Upload a JSON tool metadata file to begin."
        )
